...
// Middleware if server is accepting data from a web site so it is useful to allow a specific (fronend) url from client side to request data on server.
app.use(cors({
    origin : (origin , cb) =>{
        cb(null , origin || "*")
    } ,
...
// Middleware if server is accepting data from client in json so this middleware is helpful to assign limit so the limited amount of json data would be transfer to server from client
